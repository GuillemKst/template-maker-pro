...
// Helper function to create a welcome card template
function createWelcomeCardTemplate(name: string, role: string, timezone: string, skills: string, projects: string) {
  return {
    ...
    layout: [
      ...
      { key: 'timezone_value', x: 50, y: 260, fontSize: 16, color: '#ffffff', text: timezone, maxWidth: 500 },
      
      // Skills section
      { key: 'skills_label', x: 50, y: 340, fontSize: 20, color: '#ffd700', text: '🔨 Skills' },
      { key: 'skills_value', x: 50, y: 380, fontSize: 16, color: '#ffffff', text: skills, maxWidth: 500 },
      
      // Projects section
      { key: 'projects_label', x: 50, y: 460, fontSize: 20, color: '#ffd700', text: '🚀 Projects' },
      { key: 'projects_value', x: 50, y: 500, fontSize: 16, color: '#ffffff', text: projects, maxWidth: 500 }
    ]
  };
}
...
app.post('/api/generate-image', async (req, res) => {
  try {
    ...
    if (data.profileImage && isWelcomeCard) {
      try {
        // If it's a URL, we'd need to fetch it
        // For now, we'll add a placeholder circle for the profile image
        const profileX = config.width - 120;
        const profileY = 50;
        const profileRadius = 50;
        
        // Draw profile image placeholder circle
        ctx.fillStyle = '#ffd700';
        ctx.beginPath();
        ctx.arc(profileX, profileY, profileRadius, 0, 2 * Math.PI);
        ctx.fill();
        
        // Add profile initials or emoji
    ...
    if ((config.layout && config.layout.length > 0) || isWelcomeCard) {
      ...
      for (const item of layoutItems) {
        ...
        const value = item.text || data[item.key] || '';
        if (!value) continue;
